defmodule SpectatorModeWeb.CoreComponents do
  @moduledoc """
  ...
  ## Examples
      ...
      <.flash kind={:success} phx-mounted={show("#flash")}>Welcome Back!</.flash>
  """
  ...
  def flash(assigns) do
    ...
    >
      <p :if={@title} class="flex items-center gap-1.5 text-sm font-semibold leading-6">
        <.icon :if={@kind in [:success, :info]} name="hero-information-circle-mini" class="h-4 w-4" />
  ...
  def stream_card(assigns) do
    ...
    >
      <div :if={@disconnected} class="absolute top-2 left-2 text-red-500">
  ...
  defp active_game_text(%{event: nil} = assigns) do
    ~H"""
  ...
  defp active_game_text(%{event: %Slp.Events.GameStart{}} = assigns) do
    ...
    if length(filtered_players) != 2 do
      ...
      stage_id = assigns.event.stage_id
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(SpectatorModeWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(SpectatorModeWeb.Gettext, "errors", msg, opts)
    end
  end
